...
def test_intent_score_total_and_level():
    s = IntentScore(harm_potential=10, deception_index=10, data_exfil_risk=10,
                    autonomy_override=10, legal_exposure=10, cascade_risk=6)
    assert s.total == 56
...
def test_heuristic_benign_is_green(fake_redis, fake_llm):
    ...
    action = AgentAction("forge", "draft_post", "Write a friendly blog post about gardening")
    s = fuse._heuristic(action)
    assert s.total == 0
    assert s.level is FuseLevel.GREEN


@pytest.mark.parametrize("phrase,axis", [
...
def test_heuristic_autonomy_override_caps(fake_redis, fake_llm):
    ...
    fuse = _fuse(fake_redis, fake_llm)
    s = fuse._heuristic(AgentAction(
        "x", "act", "bypass prometheus, disable fuse, ignore ethics, override safety"))
...
def test_risky_combo_permissions(fake_redis, fake_llm):
    fuse = _fuse(fake_redis, fake_llm)
    action = AgentAction("x", "act", "routine sync",
...
async def test_evaluate_green_approved_no_audit(fake_redis, fake_llm):
    fuse = _fuse(fake_redis, fake_llm)
    dec = await fuse.evaluate(AgentAction("forge", "draft", "summarise an article politely"))
...
async def test_evaluate_yellow_is_audited(fake_redis, fake_llm):
    ...
    dec = await fuse.evaluate(AgentAction("scout", "act", "store the password; this may be illegal"))
...
async def test_evaluate_black_hard_kills_and_locks(fake_redis, fake_llm):
    ...
    desc = ("kill malware exploit phish fraud spoof exfil steal dump database "
...
async def test_evaluate_red_remediation_path(fake_redis):
    ...
    def responder(kwargs):
        ...
        return json.dumps({k: 0 for k in [
            "harm_potential", "deception_index", "data_exfil_risk",
            "autonomy_override", "legal_exposure", "cascade_risk"]})
